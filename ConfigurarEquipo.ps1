...
function Configurar-AnyDesk {
    ...
    try {
        ...
        Start-Process cmd.exe -ArgumentList "/c @echo off && for /f ""delims="" %i in ('""C:\Program Files (x86)\AnyDeskMSI\AnyDeskMSI.exe"" --get-id') do set CID=%i && echo AnyDesk ID is: ('""C:\Program Files (x86)\AnyDeskMSI\AnyDeskMSI.exe"" --get-id') && pause" -NoNewWindow
    }
    catch {
